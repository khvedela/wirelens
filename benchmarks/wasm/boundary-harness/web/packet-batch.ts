...
function inspectPacketBatch(bytes: Uint8Array): InspectedPacketBatch {
  ...
  if (view.getUint16(8, true) !== HARNESS_BATCH_SCHEMA_VERSION) {
  ...
  if (view.getUint32(12, true) !== HARNESS_API_VERSION) {
    throw invalid("packet batch API version is unsupported");
  }
  if (
    view.getUint16(16, true) !== DESCRIPTOR_BYTES ||
    view.getUint16(18, true) !== COLUMN_COUNT
  ) {
