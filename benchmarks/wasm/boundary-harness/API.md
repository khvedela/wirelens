...
Every request carries `apiVersion` and `requestId`. Packet-batch commit/discard commands identify the transfer request they resolve. Binary results use transferable `Uint8Array` payloads; all other responses use the validated control envelope in `web/worker-contract.ts`. Command API version and packet-batch schema version evolve independently as specified by ADR-0007.
