...
The dense fixture contains 60,000 short records in 15,240,024 bytes. Its proportional admission ceiling was 60,555 packets. The sparse fixture contains 256 large records in 15,367,704 bytes. Both passed through an explicit `validating` checkpoint before a later call atomically published the dataset.
...
| Worker JavaScript -> Rust ownership | 1 copy | Explicit admitted `Uint8Array.copy_to` of 15,240,024 bytes |
| Synchronous ingest peak | 2 full buffers | Transferred worker input plus Rust destination coexist only during the call |
| After `beginImport` returns | 1 full buffer | Resource stats report exactly 15,240,024 bytes Rust-owned input |
...
| Resource | Bytes |
| --- | ---: |
| Published dense capture | 15,240,024 bytes |
| Dense packet-record arena | 4,800,000 bytes |
| All dense canonical indexes and strings | 4,800,080 bytes |
| Dense retained logical total | 20,040,104 bytes |
| Parser-buffer upper bound after begin | 8,388,610 bytes |
| Packet-index reservation after begin | 9,688,800 bytes |
| Auxiliary/finalization reservation after begin | 3,159,040 bytes |
| Total logical upper bound after begin | 36,476,474 bytes |
